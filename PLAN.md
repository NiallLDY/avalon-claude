...
规则层问题见 [`GAME.md` §13](./GAME.md#13-待确认问题清单讨论用)（Q1–Q10）。此处是产品/技术层：
...
| P6 | 是否需要**深色/浅色主题切换**？ | 只做暗色 |
| P7 | 是否需要**国际化**（中/英）？ | 只做中文，文案集中在一个文件便于后加 |
| P8 | 是否要 **Cloudflare Turnstile** 建房验证？ | 先不开，预留开关 |
| P9 | 是否需要**音效**？ | 做，默认关 |
| P10 | 前端是否做 **PWA 可安装**？ | 做（单屏体验的关键） |
