...
**为什么不上 PostgreSQL**
无账号系统 ⇒ 无用户表；无跨设备数据 ⇒ 无持久化刚需。活跃房间放**内存**（唯一真相），每次状态变更**防抖写 Redis 快照**，进程重启可恢复正在进行的对局。终局战报**永久**写进 Redis 供复盘查看；另有一份公开的玩家档案与对局归档（`apps/server/src/records.ts`）撑排行榜。
...
>
> 房间码是**纯数字**：线下要念给同桌的人听，数字没有「B 还是 D」的歧义，手机上还能直接弹数字键盘。
...
| 动效 | 触发 |
|---|---|
| 投票**同时**翻面 | 揭票。刻意不做错开 —— 规则是同时公开，错开会让人以为有先后 |
| 任务牌逐张翻开 | 任务结算。**顺序打乱、牌面不对应任何人**，只演「几张失败牌」 |
| 结论大字砸下来 | 结果弹窗。过冲再回弹，匀速放大看着像加载动画 |
| 上车高亮脉冲 | 队伍成型时脉冲两下就停，一直呼吸会全程抢眼睛 |
| 队长冠落下 | 队长换人 |
| 进度点敲一下 | 该轮任务刚结算 |
| 终局横幅扫光 | 进终局页 |
| 表情包气泡 / 投掷弧线 | 见 `styles.css` 顶部那组 toss-* |
